...
for(i in 1:maxIter) {
	...
	for(x in 1:nVal) {
		...
		sl_peptide[pos] <- x
	    for(j in 1:10){
           	exp_reduc[j] <- ExpImprovement_par(train_AcpH, rbind(kg_peptides[-sl_num,], sl_peptide), AAclass, nRep = 2000)$improve
		}
	...
	for(n in 1:K) {
		...
		nm <- which.max(V)
		sl_peptide[pos] <- nm
		obs <- ExpImprovement_par(train_AcpH, rbind(kg_peptides[-sl_num,], sl_peptide), AAclass, nRep = 2000)$improve
