...
#
#Description:
# Generate a set of recommendations. To add a new recommendation to the set, Use the greedy heuristic, pick a peptide to maximize probability of improvement among a randomly generated library of peptides. 
# There are two methods for calculating prob being hit (type 1 --specifically labeled by sfp and unlabeled by PfAcpH, type 2 -- specifically labeled by AcpS and unlabeled by PfAcpH):
#       - old method is simply to treat hit as positive data and nonhit as negative data, it suffers from very limited amount of positive data.
#       - new method is to build 3 seperate classifier for a. specifically labeled by sfp; b. specifically labeled by AcpS; c. unlabeled by PfAcpH given the peptide has been labeled. Then prob being hit for type 1 is prob(a) * prob(c), and prob being hit for type 2 is prob(b) * prob(c).
#
#Input arguments:
#   training_set: $X_on is matrix of training samples for specific labeling
#                 $Y_on is vector of training labels
#                 $X_off is matrix of training sample for unlabeling
#                 $Y_off is vector of training labels
#                 note: if we use old method, then only $X_on and $Y_on is available and they are just training data for hit.
#	classlist
#             	The vector specifying the class each amino-acid belongs to.  	
#	S.Pos
#		position of serene of a peptide IN THE MODEL, this parameter is determined by the model instead of training data or recommendations to be generated.
...
#    	rec
#		a matrix with each row being the feature vector of a recommended peptide.  
maxP_search <- function(training_set, classlist, S.Pos, Nrec, itr = 500, Nlib = 1e5, maxL, maxR, minL, minR, gammas, add_ins = 1, use_new_method) {
	nF <- dim(training_set$X_on)[2]
	nAA <- length(unique(as.numeric(classlist)))
	rec <- c()
	nPep <- 0
	...
	while(nPep < Nrec) {	
		...
		rdn.peptides[rdn.peptides == 0] <- -1
		peptides.library[, (S.Pos - maxL + 1):(S.Pos + maxR)] <- rdn.peptides
		colnames(peptides.library) <- colnames(training_set$X_on)
		# train & predict
        if (use_new_method == 1) {
            prob <- Naive_Bayes(training_set$X_on, training_set$Y_on, peptides.library, classlist, S.Pos, maxL, maxR, gammas$on_negative, gammas$on_positive, predIter = itr) * Naive_Bayes(training_set$X_off, training_set$Y_off, peptides.library, classlist, S.Pos, maxL, maxR, gammas$off_negative, gammas$off_positive, predIter = itr)
        } else {
            prob <- Naive_Bayes(training_set$X_on, training_set$Y_on, peptides.library, classlist, S.Pos, maxL, maxR, gammas$on_negative, gammas$on_positive, predIter = itr) 
        }
		is_added <- FALSE
		# check the peptide to be added to recommendation set is not added before
		while(!is_added) {
			...
			if(dim(unique(rbind(rec,add_pep)))[1] > nPep){
				...
				is_added <- TRUE
				add_train_peps <- data.frame(matrix(rep(add_pep, add_ins), add_ins, byrow = TRUE))
				colnames(add_train_peps) <- colnames(trainX) #JL note: trainX has been deleted before!
				trainX <- rbind(trainX, add_train_peps)
				trainY <- c(trainY, rep(0, add_ins))	
			}
			else {
