...
for (pep_idx in 1:no_recom) {
    ...
    pep_lib <- new_opt_gen_peptide_lib(Nlib, maxL, maxR, minL, minR, S.Pos, colnames(X), alpha_label_prefer, alpha_label_prefer, alpha_unlabel, AAclass)
    ...
    label_prefer_predict_mat <- foreach (n = 1:iteration, .init = c(), .combine = rbind, .inorder=F, .errorhandling='pass') %dopar% {
        ...
        NB_predict(pep_lib, theta, S.Pos, maxL, maxR, label_prefer_prior)
    }
    ...
    label_unprefer_predict_mat <- foreach (n = 1:iteration, .init = c(), .combine = rbind, .inorder=F, .errorhandling='pass') %dopar% {
        ...
        NB_predict(pep_lib, theta, S.Pos, maxL, maxR, label_unprefer_prior)
    }
    ...
    unlabel_predict_mat <- foreach (n = 1:iteration, .init = c(), .combine = rbind, .inorder=F, .errorhandling='pass') %dopar% {
        ...
        NB_predict(pep_lib, theta, S.Pos, maxL, maxR, unlabel_prior)
    }
    unlabel_predProb <- colMeans(unlabel_predict_mat)

    prob <- label_prefer_predProb * (1-label_unprefer_predProb) * unlabel_predProb
    new_pep <- select_new_recom(rbind(X_label_prefer, X_label_unprefer, X_unlabel), pep_lib, prob)
