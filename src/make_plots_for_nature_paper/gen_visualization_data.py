...
if __name__ == '__main__':
    ...
    to_store = {}
    data_range = {}
    data_names = ["sfp_orthogonal", "AcpS_orthogonal"]
    to_store['penalties'] = penalties
    total_X = None
    cur_idx = 0
    for name in data_names:
        benchmark_seq_table = pandas.read_sql_table("benchmark_seq_"+name, engine)
        original_seq_table = pandas.read_sql_table("original_seq_"+name, engine)
        data_range["orig_"+name] = (cur_idx, cur_idx+len(original_seq_table))
        cur_idx += len(original_seq_table)
        data_range["pool_"+name] = (cur_idx, cur_idx+100)
        cur_idx += 100
        data_range["mutate_"+name] = (cur_idx, cur_idx+100)
        cur_idx += 100
        data_range["predict_optimize_"+name] = (cur_idx, cur_idx+100)
        cur_idx += 100
        X_benchmark = benchmark_seq_table.values
        X_benchmark = np.vstack(np.split(X_benchmark, [2, 4], axis=1))
        X_orig = original_seq_table.values[:,:2]
        print X_orig[0,:]
        X = np.vstack((X_orig, X_benchmark))
        if total_X is None:
            total_X = X.copy()
        else:
            total_X = np.vstack((total_X, X))
        to_store["orig_hit_"+name] = [int(x) for x in original_seq_table['hit']]
    rel_pos = compute_rel_pos(total_X, penalties)
    for key in data_range:
        to_store[key] = rel_pos[data_range[key][0]:data_range[key][1], :]
    with open("visualization_coord", "wb") as f:
