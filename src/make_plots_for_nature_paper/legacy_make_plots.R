...
hist_length <- function() {
    pdf("plots/histogram_of_length_type1.pdf", width=10, height=10)
    lengths <- c()
    for (i in 1:dim(org.data)[1]) {
        if (org.data[i, 'type1'] == 1) {
...
#### plot diverse hits, this code only produces data, TO GENERATE PDF files, run python script
diverse_hits <- function() {
...
plot_sfp_hist <- function() {
    all_datafile <- paste(root_path, "/data/all_TS_data.csv", sep='')
    all_data_org <- read.csv(all_datafile, stringsAsFactors=F)
    unlabel_count <- c()
    for (n in 1:dim(all_data_org)[1]) {
        if (all_data_org[n,'sfp'] == 1) {
            unlabel_count <- c(unlabel_count, all_data_org[n, 'PfAcpH'])
        }
    }
    plot_data_1 <- data.frame(sfp_labeling=factor(all_data_org$sfp))
    plot_data_2 <- data.frame(sfp_labeling_unlabeling=factor(unlabel_count))
    require(ggplot2)
    g <- ggplot(plot_data_1) + geom_histogram(aes(sfp_labeling), fill='orange')
    ggsave("plots/hist_sfp_labeling.pdf")
    g <- ggplot(plot_data_2) + geom_histogram(aes(sfp_labeling_unlabeling), fill='light green')
    ggsave("plots/hist_sfp_labeling_unlabeling.pdf")
}
