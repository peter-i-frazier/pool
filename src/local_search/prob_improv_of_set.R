...
#  Y <- trainY
  ...
  for (i in 1:nrow(S)) {
    ...
    # matrix to pass into CalculateProb. After conversion,
    # test.x becomes a matrix with one column. Need to transpose it
    prob.list <- CalculateProb(t(as.matrix(test.x)), thetas.1, thetas.0, p1)
