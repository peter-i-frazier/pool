...
for (pep_idx in 1:no_recom) {
    ...
    pep_lib <- foreach (n = 1:Nlib, .init = c(), .combine = rbind, .inorder=F, .errorhandling='pass') %dopar% {
        new_opt_gen_peptide(nL+nR, maxL, maxR, minL, minR, S.Pos, alpha_label_prefer, alpha_label_unprefer, alpha_unlabel, AAclass)
    }
    colnames(pep_lib) <- colnames(X_label_prefer)
    ...
    predict_mat <- foreach (n = 1:iteration, .init = c(), .combine = rbind, .inorder=F, .errorhandling='pass') %dopar% {
        ...
        new_NB_predict(pep_lib, label_prefer_theta, label_unprefer_theta, unlabel_theta, S.Pos, maxL, maxR, label_prefer_prior, label_unprefer_prior, unlabel_prior)
    }
    prob <- colMeans(predict_mat)
