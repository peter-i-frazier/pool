...
def construct_stripes(img_folder, save_path_head, img_list, num_row, num_col, row_offset):
    imp_list = []
    imp_row_list = []
    imp_col_list = []
    imp_dup_list = []
    ...
    for r in range(num_row):
        for c in range(num_col):
            ...
            imp = IJ.getImage()
            IJ.saveAs(imp, "TIFF", save_path_head + "_" + str(r+row_offset) + "_" + str(c) + ".tif")
            imp.close()
...
def rank_stripes_new(table_folder, table_name, lib_path_head, dest_folder):
    f = open(table_folder + "/" + table_name + ".csv", 'rb')
    csv_f = csv.reader(f)
    flag = False
    for l, line in enumerate(csv_f):
        ...
        if l == 2:
            flag = True
            _, r, c = parse(line1)
            imp = IJ.openImage(lib_path_head + "_"+str(r)+"_"+str(c)+".tif")
            IJ.run(imp, "Duplicate...", "title=1")
            _, r, c = parse(line)
            imp = IJ.openImage(lib_path_head + "_"+str(r)+"_"+str(c)+".tif")
            IJ.run(imp, "Duplicate...", "title=2")
...
root = "/Users/jialeiwang/Google Drive/research/Peptide Labeling_Unlabeling Project (UCSD_Cornell)/PEPTIDE RANKINGS/F_RANKINGS_TSR/2016-01-30_TSR/rank_imgs/"
#img_names = ["TSR_sfp_label", "TSR_sfp_unlabel", "TSR_AcpS_label", "TSR_AcpS_unlabel"]
#save_path_head = root + "stripes/TSR"
#construct_stripes(root+"original", save_path_head, img_names, 9, 30, 0)
...
for filename in os.listdir(root+"tables"):
    if ".csv" in filename:
        filename = filename[0:(len(filename) - 4)]
        print filename
        rank_stripes_new(root+"tables", filename, root+"stripes/TSR", root+"result_imgs")
