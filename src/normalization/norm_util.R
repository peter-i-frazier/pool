# Author: Jialei Wang
# Created: 05.01.2015
# This file provides utility functions for normalization.

require(quadprog)

GetIndex <- function(seq, seq.vec) {
  if (length(seq.vec) > 0)
...
PrepareMatrix <- function(data, const, treatment, j.class) {
  ...
  #   j.class: vector of training sets
  #
  # Returns:
  #       D, d, A, b.0, dup_unique_seqs
  ij.idx <- matrix(NA, nrow=nrow(data), ncol=2)
  colnames(ij.idx) <- c('i', 'j')
  unique_seqs <- c()
  for (n in 1:nrow(data)) {
      if (GetIndex(data[n, 'seq'], unique_seqs) == 0)
          unique_seqs <- c(unique_seqs, data[n, 'seq'])
      ij.idx[n, 'i'] <- GetIndex(data[n, 'seq'], unique_seqs)
      ij.idx[n, 'j'] <- data[n, 'TS']
  }
  ...
  dvec <- rep(0, len_x)
  for (n in 1:nrow(data)) {
    beta_idx <- which(j.class == ij.idx[n, 'j'])
    theta_idx <- ij.idx[n, 'i'] + length(j.class)
    y <- data[n, treatment]
...
Normalize <- function(org.data, dup.data, treatment, result.path=NA, const=NA) {
  ...
  matrix.list <- PrepareMatrix(dup.data, const, treatment, j.class)
  D <- matrix.list$D
  d <- matrix.list$d
  A <- matrix.list$A
  b.0 <- matrix.list$b.0
  ...
  for (n in 1:nrow(org.data)) {
    norm.data[n, 'unique_seq_idx'] <- GetIndex(norm.data[n, 'seq'], unique.seqs)
    norm.data[n, 'raw'] <- org.data[n, treatment]
    norm.data[n, 'sigma'] <- 1 / solution[which(j.class == norm.data[n, 'TS'])]
    dup.idx <- GetIndex(norm.data[n, 'seq'], dup.unique.seqs)
    if (dup.idx == 0) {
