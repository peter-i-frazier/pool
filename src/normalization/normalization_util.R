...
prepare_matrix <- function(data, k_class, j_class, group_name, nolabel_table, not_unlabel_table, const) {
...
# min x^T D x, s.t A^T x >= 0
    ...
    A_nolabel <- c()
    # theta_ik1 - theta_ik2 = 0 for not_label
    ...
    #meq <- 1 + length(nolabel_i_idx) + length(not_unlabel_i_idx)
    ...
    A_matrix <- rbind(A_sum_theta, A_nolabel, A_not_unlabel, A_positive, A_theta)
    # b_0
    b_0 <- c(const, rep(0, dim(A_matrix)[1]-1))
    ...
    # eq_matrix <- A_unlabel
    return (list(D=D_matrix, A=t(A_matrix), b_0=b_0, meq=meq, unique_seqs=unique_seqs))
}
