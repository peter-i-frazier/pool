...
#--------------------------------------------------------------------------------
    ...
    is.found <- foreach(icount(nRep), .combine = c, .init = c(), .packages = c('Rlab'),
	    .multicombine=TRUE, .export = c('NB_predict','getTheta_MC')) %dopar% {        
        theta <- getTheta_MC(alpha = alpha, classlist = classlist)
		prob.hit <- NB_predict_par(newdata, theta, prior.positive = prior.positive)
