...
To describe our optimal learning approach, and to contrast it more precisely with the 
previously described method of taking the peptides with the best predictions (which 
we call the \enquote{predict-then-optimize} approach for short), we introduce mathematical 
notation to formulate the peptide optimization problem. 

Let $E$ be the set containing all possible peptide sequences, and for any peptide 
sequence $e \in E$, let $y(e) \in \{0, 1\}$ be the binary label indicating whether
peptide $e$ is active or not, if it were measured. 
...
The goal is to use machine learning algorithm and training data to find active peptides. To
make the goal more general, suppose we are looking for active peptides with some
favored property, and the measure of favorability can be determined by a fitness 
function $f(e)$. Then the optimization problem of finding favored active peptides
can be written as
\begin{equation}
